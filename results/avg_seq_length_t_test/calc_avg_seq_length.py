...
pattern = re.compile('sigOrfs_seq_length_p.*\.txt')
#pattern = re.compile('sig_pruned_masked_p\.05_rcclust_length\.txt')
LENGTH,SIG_COUNT,NSIG_COUNT = range(3)	# 0, 1, 2

file_out = open('/afs/crc.nd.edu/user/k/kngo/leRCC/results/avg_seq_length_t_test/avg_seq_length.txt', 'w') # overwrites if exits

file_out.write('Calculated average seq length for significant and non-significant sequences differing by p-Value cut offs.')
file_out.write('\n')
file_out.write('\n')

for path, subdirs, files in os.walk("/afs/crc.nd.edu/user/k/kngo/leRCC/results/avg_seq_length_t_test"):
	...
	for filename in files:
		if pattern.match(filename):
			...
			file_out.write('sig avg seq length: ' + str(round(sig_avg_length,2)))
			file_out.write('\n')
			file_out.write('non-sig avg seq length: ' + str(round(nsig_avg_length,2)))
			file_out.write('\n')
			file_out.write('\n')
