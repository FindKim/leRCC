...
print med
'''
pattern = re.compile('sig_pruned_masked_p.*_rcclust_length\.txt')
#pattern = re.compile('sig_pruned_masked_p\.05_rcclust_length\.txt')
LENGTH,SIG_COUNT,NSIG_COUNT = range(3)	# 0, 1, 2

file_out = open('/afs/crc.nd.edu/user/k/kngo/leRCC/results/sigOrfs_masked/rcclust_length/median_rrclust_length.txt', 'w') # overwrites if exits

file_out.write('Calculated median rare codon cluster length for significant and non-significant sequences differing by p-Value cut offs.')
file_out.write('\n')
file_out.write('\n')
...
for path, subdirs, files in os.walk("/afs/crc.nd.edu/user/k/kngo/leRCC/results/sigOrfs_masked/rcclust_length"):
	...
	for filename in files:
		if pattern.match(filename):
			...
			file_out.write('sig median rcc length: ' + str(round(sig_median,2)))
			file_out.write('\n')
			file_out.write('non-sig median rcc length: ' + str(round(nsig_median,2)))
			file_out.write('\n')
			file_out.write('\n')
