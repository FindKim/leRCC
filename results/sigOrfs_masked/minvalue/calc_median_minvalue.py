...
MINVALUE,SIG_COUNT,NSIG_COUNT = range(3)	# 0, 1, 2

file_out = open('/afs/crc.nd.edu/user/k/kngo/leRCC/results/sigOrfs_masked/minvalue/median_minvalue.txt', 'w') # overwrites if exits

file_out.write('Calculated median %min value for significant and non-significant sequences differing by p-Value cut offs.')
file_out.write('\n')
file_out.write('\n')
...
for path, subdirs, files in os.walk("/afs/crc.nd.edu/user/k/kngo/leRCC/results/sigOrfs_masked/minvalue"):
	...
	for filename in files:
		if pattern.match(filename):
			...
			with open(file_path) as f:
				...
				for line in f:
					...
					for i in range(0, int(line[SIG_COUNT])):
						sig_length_list.append(int(line[MINVALUE]))
					for i in range(0, int(line[NSIG_COUNT])):
						nsig_length_list.append(int(line[MINVALUE]))
			...
			file_out.write('sig median %min value: ' + str(round(sig_median,2)))
			file_out.write('\n')
			file_out.write('non-sig median %min value: ' + str(round(nsig_median,2)))
			file_out.write('\n')
			file_out.write('\n')
